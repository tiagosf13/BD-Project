    ...
    try:
        ...
        if not result[0][0]:
            if table_name == "users":
                ...
                query = "CREATE TABLE users (id SERIAL PRIMARY KEY, username VARCHAR(255), password VARCHAR(255), reset_token VARCHAR(255), reset_token_timestamp TIMESTAMP, email VARCHAR(255), secret_key VARCHAR(255), secret_key_timestamp TIMESTAMP, admin BOOLEAN)"
                db_query(query, ())
